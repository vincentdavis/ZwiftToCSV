...
if "https://zwiftpower.com/profile.php?z=" in profile_url:
    ...
    wkg_curve.update_layout(
        title="WKG and Coggan Levels",
        height=700,
    )
    for i, w in enumerate(cf.Level.unique()):
        c = cf[cf.Level == w]
        wkg_curve.add_trace(
            go.Scatter(name=f"{w}", x=c["time"], y=c[gender], mode="lines", fill="tonexty", line_color=colors[-i])
        )
    wkg_max = {"wkg_ftp": 0, "wkg1200": 0, "wkg300": 0, "wkg60": 0, "wkg30": 0, "wkg15": 0, "wkg5": 0}
    wkg_max.update(df[wkg_list].max().to_dict())
    print(wkg_max)
    wkg_curve.add_trace(
        go.Scatter(name="Athlete's WKG", x=x, y=list(wkg_max.values()), mode="lines", line_color="blue")
    )
